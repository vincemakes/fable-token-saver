...
from `assets/agents/codex/` as `model-boss-<role>.toml` under `.codex/agents/`
(project) or `$HOME/.codex/agents/` (user). Each TOML supplies a spawned role's model,
reasoning effort, sandbox mode, and instructions; it does not configure the main loop.
...
catalog, and that the requested sandbox and reasoning settings are accepted. Model
Boss does not automatically upgrade the CLI.
...
Codex can invoke installed `claude-kimi*` and `claude-glm*` commands through Model
Boss's external adapter. This does not claim that Kimi Chat Completions is a native
...
is disabled and Web/MCP are unavailable. Model Boss runs gates in the host. The
